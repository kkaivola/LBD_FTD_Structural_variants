...
#Extract each FTD sample of interest separately from 1% FDR GATK-SV data, include only SVs with an alternative allele (=exclude 0/0s) 
...
for SAMPLE in $FTD_SAMPLES; do
    ...
    -s $SAMPLE \
    -Ou \
    /data/ALS_50k/DementiaSeq_SV.LBD.FTD/CLEAN.SV.fromJinHui/FTD_gatksv_1perc_fdr_cleaned_filters_qual_recalibrated/FTD_1perc_fdr.cleaned_filters_qual_recalibrated.LNGsampleID.vcf.gz \
    | \
    bcftools view -i 'GT[*]="alt"' \
    -Oz \
